...
class FourLayerNet(object):
    ...
    The architecure is affine - relu - affine - relu - affine - relu - affine - softmax.
    ...
    def __init__(
        ...
        self.params['W1'] = np.random.normal(scale=weight_scale, size=(input_dim, hidden_dim))
        self.params['W2'] = np.random.normal(scale=weight_scale, size=(hidden_dim, hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        out_layer3, cache_layer3 = affine_relu_forward(out_layer2, self.params["W3"], self.params["b3"])
        scores, cache_layer4 = affine_forward(out_layer3, self.params["W4"], self.params["b4"])
        ...
        # L2 regularization is L2 norm of the weights (sum of squares)
        regularization = np.sum(np.square(self.params["W1"])) + np.sum(np.square(self.params["W2"])) +  np.sum(np.square(self.params["W3"])) + np.sum(np.square(self.params["W4"])) 
        loss_softmax, dx_softmax = softmax_loss(scores, y) 
        ...
        # backward pass (computing gradients)
        dx_affine_backward3, grads['W4'], grads['b4'] = affine_backward(dx_softmax, cache_layer4)
        dx_affine_backward2, grads['W3'], grads['b3'] = affine_relu_backward(dx_affine_backward3, cache_layer3) 
        dx_affine_backward1, grads['W2'], grads['b2'] = affine_relu_backward(dx_affine_backward2, cache_layer2) 
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dx_affine_backward1, cache_layer1) 
